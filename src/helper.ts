...
export class KookCard {
  static searchModrinthCard(
    ...
    for (const project of hits) {
      const newSection = {
        ...
        text: {
          type: "kmarkdown",
          content: `[${project.title}](https://modrinth.com/project/${
            project.project_id
          }) | 作者 **${project.author}**\n**类别** ${project.categories} | **版本** ${
            project.versions[0]
  ...
  static UserCard(name, uuid, oldHead, newHead, skin) {
    return [
      {
        type: "card",
        size: "lg",
